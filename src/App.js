...
function App() {
      ...
      if (!shuffleArray[i][j].bomb) {
        ...
        for (let k = i - 1; k < i + 2; k++) {
          for (let p = j - 1; p < j + 2; p++) {
            if (k > -1 && k < map_row && p > -1 && p < map_col)
              shuffleArray[k][p].count++;
          }
        }
        bombCount--;
      }
  ...
  const Area = (props) =>
    table[props.i][props.j].isOpen ? (
      table[props.i][props.j].bomb ? (
        <Button w="100%" h="100%" borderRadius="0" bg="#C56869">
          <Image src={bombIcon} w="27px" h="24px" />
        </Button>
      ) : (
        <Button w="100%" h="100%" borderRadius="0" bg="#CDC6C3">
          {table[props.i][props.j].count}
        </Button>
      )
    ) : (
      <Button
        ...
        borderRadius="0"
        bg="white"
        className={`button_${props.i}_${props.j}`}
  ...
  return (
    ...
    >
      ...
      >
        {table &&
          table.map((row, i) =>
            row.map((col, j) => <Area i={i} j={j} key={i * 10 + j} />)
          )}
      </Grid>
      <Modal isOpen={isOpen}>
        <ModalOverlay />
        <ModalContent
          mt="40vh"
          bg={lose ? "#E9524D" : "#FFD7DF"}
          boxShadow="dark-lg"
          textAlign="center"
          maxW="60vw"
        >
          <ModalHeader className="header" fontSize="xx-large" margin="4vh">
            {lose ? "Game over!!!" : "You win!!!"}
          </ModalHeader>
          <ModalBody fontSize="large">
            {lose
              ? 'Press "try again" button to play!'
